...
_mysql_cnf() {
    # _mysql_cnf <password>  →  prints path to a chmod-600 .cnf
    local pass="$1" cnf_dir="${TMP_DIR}/.mysql_cnf"
    mkdir -p "${cnf_dir}"
    local cnf="${cnf_dir}/$$_$(date +%s%N).cnf"
    cat > "${cnf}" <<CNFEOF
[client]
password=${pass}
CNFEOF
...
mysql_host_run() {
    ...
    [ -n "${db}" ] && args+=("${db}")
    local out errf err="" cnf
    cnf="$(_mysql_cnf "${pass}")"
    args=(--defaults-file="${cnf}" "${args[@]}")
    errf="$(mktemp)"
    out="$(mysql "${args[@]}" -e "${sql}" 2>"${errf}" || true)"
    rm -f "${cnf}"
    if [ -s "${errf}" ]; then
        err="$(tr '\n' ' ' < "${errf}" | sed 's/  */ /g')"
    fi
...
check_database_quality() {
    ...
    out="$(mysql_cli_run "${DB_HOST}" "${DB_PORT}" "${DB_USER}" "${DB_PASS}" "" \
            "SELECT SCHEMA_NAME FROM information_schema.SCHEMATA WHERE SCHEMA_NAME=${db_ident};")"
    if [ "${out}" != "${DB_NAME}" ]; then
    ...
    local db_ident_charset
    db_ident_charset="$(sql_escape_ident "${DB_NAME}")"
    out="$(mysql_cli_run "${DB_HOST}" "${DB_PORT}" "${DB_USER}" "${DB_PASS}" "${DB_NAME}" \
        "SELECT DEFAULT_CHARACTER_SET_NAME FROM information_schema.SCHEMATA WHERE SCHEMA_NAME=${db_ident_charset};")"
    case "${out}" in
        utf8mb4)
            ok "字符集：utf8mb4"
            ;;
        ...
        *)
            ...
            if [ "$(ask_yes_no "尝试自动 ALTER DATABASE 修复字符集？" "Y")" = "y" ]; then
                local alter_out db_ident_alt
                db_ident_alt="$(sql_escape_ident "${DB_NAME}")"
                alter_out="$(mysql_cli_run "${DB_HOST}" "${DB_PORT}" "${DB_USER}" "${DB_PASS}" "" \
                    "ALTER DATABASE ${db_ident_alt} CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;")"
                if [ -n "${alter_out}" ]; then
...
prompt_db_docker() {
    ...
    #   2. We clean up the secrets dir after container starts
...
EOF
    ...
    errf="$(mktemp)"
    local root_cnf; root_cnf="$(_mysql_cnf "${root_pass}")"
    if ! mysql --defaults-file="${root_cnf}" -h"${DB_HOST}" -P"${DB_PORT}" -u"${root_user}" --connect-timeout=10 \
            < "${sql_file}" 2>"${errf}"; then
        create_out="$(cat "${errf}")"
        rm -f "${sql_file}" "${errf}" "${root_cnf}"
...
write_application_yml() {
    ...
    # Restrict keys directory — contains PEM private keys
    chmod 700 "${KEYS_DIR}"
